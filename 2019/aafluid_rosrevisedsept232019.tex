...
\RUNAUTHOR{Aghajani and Williams}

\RUNTITLE{Fluid limit of a multi-class queue with random order of service}

\TITLE{Fluid Limit for a Multi-Class $G/G/1+G$ Queue with Random Order of Service}
...
\AFF{Google, \EMAIL{raghajani01@gmail.com}}
\AUTHOR{Ruth J. Williams}
\AFF{Department of Mathematics, University of California San Diego, 9500 Gilman Dr, La Jolla, CA 92093, \EMAIL{rjwilliams@ucsd.edu}}
% Enter all authors
} % end of the block
...
\KEYWORDS{Fluid limit, random order of service, queueing system, measure-valued processes.}
...
\]
Since $\epsilon>0$ was arbitrary, the desired result follows on letting $\epsilon\to 0$.\Halmos 
\endproof
...
\end{align}
The equation \eqref{nufchange} follows on substituting \eqref{temp_wchange1}-\eqref{temp_wchange2} in \eqref{temp_wchange0}.\Halmos 
\endproof
...
\end{proposition}
\proof{Proof.}
The result follows from Theorem T7, comment ($\beta$) below the theorem in  \cite[page 61]{BremaudBook}, and the fact that  the sequence $\{\tau_k;k\geq 1\}$ of event times is non-explosive, almost surely (see Remark \ref{def_tomega}).\Halmos 
\endproof
...
\end{lemma}
\proof{Proof.}
The proof is elementary and follows from  \cite[Lemma II.L3]{BremaudBook} and \cite[Equation (18.1) in Chapter IV]{RogWilbook2}).\Halmos 
\endproof
...
\end{align*}
is an $\{\filt_t\}$-intensity of $\reneg_{\ell,j}$.\Halmos 
\endproof
...
\end{align*}
is an $\{\filt_t\}$-intensity of $\entry^Q_{\ell,j}$.\Halmos 
\endproof
...
\]
which in turn shows that, uniformly on compact sets,  $\{\bar M^m_{R,\ell}(f;\cdot)\}_m$ converges to zero in probability, and hence  in distribution.\Halmos 
\endproof
...
\]
which in turn shows that, uniformly on compact sets,  $\{\bar M^m_{S,\ell}(f;\cdot)\}_m$ converges to zero in probability, and hence  in distribution.\Halmos 
\endproof
...
\end{proposition}
\proof{Proof.}
The necessity and sufficiency of K1 and K2a follow from \cite[Theorem 3.7.2]{KurtzBook} and the sufficiency of K1 and K2b follows from \cite[Theorems 3.7.2 and 3.8.6 and Remark 3.8.7]{KurtzBook}.\Halmos 
\endproof
...
\end{proposition}
\proof{Proof.}
The result follows from applying Theorem 13.4 in \cite{BillingsleyBook} to any converging subsequence of $\{\pi^m\}_{m\in\N}$.\Halmos 
\endproof
...
\proof{Proof.}
  ...
  \end{equation}
  Equation \eqref{compact_contain} with $M>2C_t(1+\gamma_\ell)/(\gamma_\ell\eta)$ then follows from \eqref{temp_cc1} and \eqref{temp_cc2}, and the definitions of $\tilde{\mathcal{C}}_M$ and $\mathcal{C}_M$.\Halmos 
\endproof
...
\]
and hence condition K2a holds with $\beta=1$.\Halmos 
\endproof
...
Finally, tightness for sequence $\{\bar A^m_\ell(f;\cdot)\}_{m\in\N}$, $\{\bar M^m_{R,\ell}(f;\cdot)\}_{m\in\N}$, and $\{\bar M^m_{S,\ell}(f;\cdot)\}_m$ is proved in Lemmas  \ref{lem_Atight}, \ref{lem_MRconv}, and \ref{lem_MSconv}, respectively. This completes the proof.\Halmos 
\endproof
...
\end{equation}
and hence $J(\bnu^m)\Rightarrow 0$, as $m\to\infty$. $\mathcal{C}$-tightness of the sequence $\{\bnu^m\}_{m\in\N}$ then follows from Proposition \ref{prop_Ctight_cond}.\Halmos 
\endproof
...
\proof{Proof.}
  ...
  \]
Since the function $u\mapsto \langle f,\bnu_\ell(u) \rangle $ is non-negative and differentiable at $t$, its derivative at $t$ needs to be zero.\Halmos 
\endproof
...
The last claim then follows from the display above with  $\epsilon=\bar Q(t)/4L\Lip_{\f1}>0$.\Halmos 
\endproof
...
\end{equation}
which proves \eqref{Kdot}.\Halmos 
\endproof
...
Putting all the cases together, we conclude that $\nextarrival_{k+1},\ell=1,...,L$, $\renegtime_{\ell,j}, \ell=1,...,L$, $j\in\inque_\ell(\tau_k)$, and $\nextdeparture_{k+1}$ are conditionally independent given $\filt_{\tau_k}$. Moreover, \eqref{cond_density_reneg} follows on summing \eqref{cond_density_reneg_1}, \eqref{cond_density_reneg_2}, \eqref{cond_density_reneg_3},\eqref{cond_density_reneg_4}, and \eqref{cond_density_reneg_5} over all $\ell=1,...,$ and $j\in\Z$, and \eqref{cond_density_departure} follows on summing \eqref{cond_density_departure_1}, \eqref{cond_density_departure_2}, \eqref{cond_density_departure_3}, \eqref{cond_density_departure_4}, and \eqref{cond_density_departure_5} over all $\ell=1,...,$ and $j\in\Z$.
 ...
 Finally, given $\filt_{\tau_k}$, $\selector_{K(\tau_k)+1}$ is drawn from an i.i.d.\ sequence of uniformly distributed random variables, that are independent of all other random variables, and hence, \eqref{cond_density_selection} is satisfied.\Halmos 
\endproof
